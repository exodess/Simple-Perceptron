...
TEST(MatrixPerceptronTest, test_1) {
    Matrix_perceptron p(2);
    std::vector<float> test_weights = p.GetWeights();
    EXPECT_EQ(p.hiddenLayers(), 2);
    for (auto& x : test_weights) {
        EXPECT_NE(0.000000, x);
    }
}

TEST(MatrixPerceptronTest, test_2) {
    Matrix_perceptron p(2);
...
TEST(MatrixPerceptronTest, test_3) {
    Matrix_perceptron p(2);
    std::vector<float> image(784, 0.5f);
    int result = p.Verify(image);
...
TEST(MatrixPerceptronTest, test_4) {
    Matrix_perceptron p(2);
...
TEST(MatrixPerceptronTest, test_5) {
